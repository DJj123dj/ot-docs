...
The options are the <ColorText color="red">**variables that create the buttons**</ColorText> in your ticket bot. Every button can be configured as much as you want!
...
If you are ready with the options, you can move on to the <InlineLink url="docs/config/main/messages" isdoc="true">**Message Settings**</InlineLink> below to create embeds!

## Basic Option \{#basic-option}
The basic option does not really exist, but the settings you will see here are <ColorText color="red">**required in every option!**</ColorText>
The `"id"` is a unique id for every option in Open Ticket. <ColorText color="blue">**There can't be any options with the same id!**</ColorText>
...
⚠️ **You will need this id later to <InlineLink url="docs/config/main/messages" isdoc="true">configure the messages!</InlineLink>**
