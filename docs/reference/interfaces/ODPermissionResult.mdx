---
title: ODPermissionResult
description: "The result returned by `ODPermissionManager.getPermissions()`."
keywords: [open-ticket, open ticket, api reference, discord ticket bot, interface]
sidebar_class_name: otdocs-badge-page otdocs-meta-gray-inter
---
<head>
    <link rel="apple-touch-icon" href="/img/logo_vertical.png"/>
</head>
import {C,M,ScaleImage,LinkBlock,InlineLink,FlexHorizontal,Divider,Hex,ApiUrl,ApiBlock} from "@site/src/components/openticket.jsx"
import Tabs from "@theme/Tabs"
import TabItem from "@theme/TabItem"

# ODPermissionResult <M color="gray">interface</M>

:::warning under construction
We are still working the **API Reference**! This page might be incomplete or doesn't contain all details!
:::

{/*
:::danger deprecated
This interface has been flagged as deprecated and will be removed in the next version of Open Ticket.

**Use `SomeOtherInterface` instead!**
:::

:::info experimental
This interface is flagged as experimental and might not be available in all Open Ticket versions.

**Available for Open Ticket `v4.0.0` and above.**
:::

:::warning
This is an example warning for this interface. Explain something here which should require a lot of attention!
:::

:::tip
This is an example tip for this interface. Explain something here which could help when using this interface!
:::
*/}

## Overview
The result returned by `ODPermissionManager.getPermissions()`.

> [**View Source**](https://otgithub.dj-dj.be/tree/main/src/core/api/modules/permission.ts#L21)

<FlexHorizontal>
<div style={{width:"50%"}}>
    #### Properties \{#overview-properties}
    <ul>
        <li>**[`level`](#prop-level)**</li>
        <li>**[`scope`](#prop-scope)**</li>
        <li>**[`source`](#prop-source)**</li>
        <li>**[`type`](#prop-type)**</li>
    </ul>
</div>
<div style={{width:"50%"}}>
    #### References \{#overview-references}
    <ul>
        {/*
        //TODO
        <li>**<code><ApiUrl url="class:ODId" label="api.ODId"/></code> <M color="purple">class</M>**</li>
        <li>**<code><ApiUrl url="type:ODExampleType" label="api.ODExampleType"/></code> <M color="brown">type</M>**</li>
        */}
    </ul>
</div>
</FlexHorizontal>

## Properties
<Divider></Divider>
#### `level` <M color="yellow">enum</M> <ApiBlock><ApiUrl url="enum:ODPermissionLevel" label="api.ODPermissionLevel"/></ApiBlock> \{#prop-level}
The highest level available for this scope.

> [**View Source**](https://otgithub.dj-dj.be/tree/main/src/core/api/modules/permission.ts#L27)

<Divider></Divider>
#### `scope` <M color="brown">type</M> <ApiBlock>"default"|<ApiUrl url="type:ODPermissionScope" label="api.ODPermissionScope"/></ApiBlock> \{#prop-scope}
The permission scope.

> [**View Source**](https://otgithub.dj-dj.be/tree/main/src/core/api/modules/permission.ts#L25)

<Divider></Divider>
#### `source` <M color="brown">type</M> <ApiBlock><ApiUrl url="js:null" label="null"/>|<ApiUrl url="class:ODPermission" label="api.ODPermission"/></ApiBlock> \{#prop-source}
The permission which returned this level.

> [**View Source**](https://otgithub.dj-dj.be/tree/main/src/core/api/modules/permission.ts#L29)

<Divider></Divider>
#### `type` <M color="brown">type</M> <ApiBlock><ApiUrl url="type:ODPermissionType" label="api.ODPermissionType"/></ApiBlock> \{#prop-type}
The permission type.

> [**View Source**](https://otgithub.dj-dj.be/tree/main/src/core/api/modules/permission.ts#L23)

<Divider></Divider>
