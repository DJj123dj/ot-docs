---
title: ODDefaultsNumbers
description: "This type is a list of number defaults available in the `ODDefaultsManager` class. It's used to generate typescript declarations for this class."
...
# ODDefaultsNumbers <M color="brown">type</M>
...
> [**View Source**](https://otgithub.dj-dj.be/tree/main/src/core/api/modules/defaults.ts#L207)
...
## Definition
#### <ApiBlock>([Key in (keyof <ApiUrl url="interface:ODDefaults" label="api.ODDefaults"/>)]: (<ApiUrl url="interface:ODDefaults" label="api.ODDefaults"/>[Key] extends <ApiUrl url="js:number" label="number"/> ? Key : never))[(keyof <ApiUrl url="interface:ODDefaults" label="api.ODDefaults"/>)]</ApiBlock> {#definition-code}
